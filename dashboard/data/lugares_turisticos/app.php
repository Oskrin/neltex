<?php
	...
	if (isset($_POST['eliminar_registro'])) {
		$acu[0] = 1;//no actualizado
		$resultado = $class->consulta("UPDATE lugar_turistico SET stado='0' WHERE id='$_POST[id]';");
		if (!$resultado) {
			$acu[0] = 0;// actualizado
		}
	}
	...
	if (isset($_POST['llenar_lugares'])) {
		$resultado = $class->consulta("SELECT * FROM lugar_turistico where id = '$_POST[id]'");
		while ($row = $class->fetch_array($resultado)) {
			$data = array('id' => $row[0], 
						'nombre' => $row[1],
						'propietario' => $row[2],
						...
						'descripcion' => $row[9],
						'foto' => $row[10],
						'id_tipo_lugar_turistico' => $row[11],
						'id_parroquia' => $row[12]	
						);
		}
		print_r(json_encode($data));
