<?php
    ...
    class PDF extends FPDF {   
        ...
        function Header() {             
            ...
            $this->Cell(150, 5, "TURISTAS", 0,1, 'R', 0);      
            ...
            $this->Cell(50, 5, utf8_decode("CORREO"),1,0, 'C',0);
            $this->Cell(30, 5, utf8_decode("TELÉFONO"),1,0, 'C',0); 
            $this->Cell(57, 5, utf8_decode("DIRECCIÓN"),1,1, 'C',0); 
        }
        function Footer() {            
            $this->SetY(-15);            
    ...
    $pdf->SetX(5);
    $resultado = $class->consulta("SELECT * FROM turista");
    $pdf->SetFont('Amble-Regular','',9);   
    $pdf->SetX(5); 
    while ($row=$class->fetch_array($resultado)) {                  
        $pdf->SetX(1);                  
        $pdf->Cell(70, 5, utf8_decode($row[1]),0,0, 'L',0);
        $pdf->Cell(50, 5, utf8_decode($row[2]),0,0, 'L',0);
        $pdf->Cell(30, 5, utf8_decode($row[4]),0,0, 'L',0);
        $pdf->Cell(57, 5, utf8_decode($row[5]),0,0, 'L',0);               
        $pdf->Ln(5);        
    }    
