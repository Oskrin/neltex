<?php
    ...
    if (!$sidx)
        $sidx = 1;
    $result = pg_query("SELECT COUNT(*) AS count FROM detalle_pagos_venta WHERE id_pagos_venta = '".$_GET['id_pagos_venta']."'");
    ...
    if ($search == 'false') {
        $SQL = "SELECT * FROM detalle_pagos_venta WHERE id_pagos_venta = '".$_GET['id_pagos_venta']."' and estado = 'Activo' ORDER BY  $sidx $sord offset $start limit $limit";
    } else {
